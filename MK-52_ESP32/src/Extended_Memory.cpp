...
bool Extended_Memory::replaceLine(char *line){
    if( line == NULL) line = _text;
...
bool Extended_Memory::replaceLine_P(const char *line){
...
bool Extended_Memory::insertLine(char *line){
    if( line == NULL) line = _text;
    size_t toCopy = strlen(line) + 1;
    if( _pointer + toCopy >= _limit) return false; // no space
    if( _pointer >= _bottom){
...
bool Extended_Memory::insertLine_P(const char *line){
    size_t toCopy = strlen(line) + 1;
    if( _pointer + toCopy >= _limit) return false; // no space
    if( _pointer >= _bottom){
        if( _pointer+toCopy >= _limit) return false;
...
bool Extended_Memory::updateLine(char *line){
    if( EditOverwrite) replaceLine(line);
    else insertLine(line);
}

bool Extended_Memory::updateLine_P(const char *line){
    if( EditOverwrite) replaceLine_P(line);
    else insertLine_P(line);
}


void Extended_Memory::deleteLine(){
...
char *Extended_Memory::appendText( char *text){
...
char *Extended_Memory::appendText_P( const char *text){
    int8_t ln = strlen(_text);
    char *ptr = _text + ln;
