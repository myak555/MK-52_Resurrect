...
// Inits number entry
//
unsigned long NumberReceiver::init( void *components[]) {
    _kbd = (KBDManager *)components[COMPONENT_KBD_MANAGER];
    return millis(); 
}

void NumberReceiver::activate( uint8_t scancode){
    strcpy_P( _text, PSTR(" "));
    _mode = 1;
    if(!scancode) return;
...
void NumberReceiver::tick(){
    uint8_t scancode = _kbd->scan();
    if( !scancode) return;
    _appendChar( _convertButton( scancode));
}

char NumberReceiver::_convertButton(uint8_t scancode){
    char c = (char)pgm_read_byte(_NR_ButtonConversion + scancode);
    if( c == '#') return 0;
    return c;
}

void NumberReceiver::_appendChar( char c){
    int ln = strlen(_text);
    switch( c){
      case 0:
          return;
