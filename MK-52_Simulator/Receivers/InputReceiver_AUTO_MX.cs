...
namespace MK52Simulator.Receivers
{
    ...
    //
    public class InputReceiver_AUTO_MX: RPN_InputReceiver
    {
        public InputReceiver_AUTO_MX(RPN_Calculator parent, RPN_Screen display)
            : base( parent, display)
        {
            Moniker = "AUTO_MX";
            DisplayName = "M-->X";
        ...
        public override bool onButton(RPN_Button button)
        {
            switch (button.Moniker)
            {
                ...
                case "GOTO":
                case "GOSUB":
                    _parent.Stack.SetStackFromMemory(button.Register);
                    _parent.setReceiver("AUTO_N");
                    return true;
                default:
                    return false;
